...
calcoffsets(void)
{
	...
	if (lines > 0)
		...
		n = mw - (promptw + inputw + TEXTW(prevsymbol) + TEXTW(nextsymbol));
...
drawmenu(void)
{
	...
	if (lines > 0) {
		...
		if (curr->left) {
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_text(drw, x, 0, w, bh, lrpad / 2, prevsymbol, 0);
		}
		x += w;
		for (item = curr; item != next; item = item->right)
			x = drawitem(item, x, 0, MIN(TEXTW(item->text), mw - x - TEXTW(nextsymbol)));
		if (next) {
			w = TEXTW(nextsymbol);
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_text(drw, mw - w, 0, w, bh, lrpad / 2, nextsymbol, 0);
		}
	}
